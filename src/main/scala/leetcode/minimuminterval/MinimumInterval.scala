package leetcode.minimuminterval
...
object MinimumInterval {
  ...
  private def findValue(lookupTree: mutable.TreeMap[Int, Int], query: Int): Int =
    lookupTree.getOrElse(query, lookupTree.maxBefore(query).get._2)

  private def buildTree(intervals: Array[Interval]): mutable.TreeMap[Int, Int] = {
    val tree = mutable.TreeMap(0 -> -1)
    intervals.foreach(processInterval(tree, _))
    println(tree)
    tree
  }

  private def processInterval(tree: mutable.TreeMap[Int, Int], interval: Interval): Unit = {
  ...
  private def updatePosition(tree: mutable.TreeMap[Int, Int], interval: Interval, position: Int, value: Int, prevValue: Int): Int = {
    val res =
