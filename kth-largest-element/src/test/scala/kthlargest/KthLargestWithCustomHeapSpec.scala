...
import kthlargest.KthLargestWithCustomHeap
import scala.reflect.ClassTag

class KthLargestWithCustomHeapSpec extends AnyWordSpec with Matchers {
  def L[T: ClassTag](values: T*) = List.apply(values:_*)

  "KthLargestWithCustomHeap" should {
    "compute correctly" in {
      ...
      forAll (values) {
        (k: Int, initial: List[Int], queries: List[Int], results: List[Int]) => {
          ...
          val obj = new KthLargestWithCustomHeap(k, initial)
